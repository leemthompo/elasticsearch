...
<<esql,Elasticsearch Query Language ({esql})>> is a piped query language for searching, filtering, transforming, and analyzing data.
...
Today, it supports a subset of the features available in Query DSL.
Since 8.16.0, {esql} supports a subset of <<esql-limitations-full-text-search,full-text search capabilities>>.
...
Full-text search support since 8.16.0.
| <<esql-rest,`_query`>>
