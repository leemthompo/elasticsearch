...
This page covers the full API surface, settings, and operational details for {{esql}} federated data sources and datasets. The API is organized around two core objects: data source APIs manage connections to external storage, and dataset APIs manage the queryable resources within those connections. Dataset settings control compression and format-reader behavior. For an introduction to these concepts, refer to [{{esql}} federated data](esql-federated-data.md). For a quickstart guide, refer to [](esql-federated-data-quickstart.md).
